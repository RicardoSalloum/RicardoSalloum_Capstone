...
def run_exam(sid: int) -> dict:
    ...
    for i, q in enumerate(exam_questions, 1):
        ...
        mark = "✓" if is_correct else ("~" if (conf < CONFIDENCE_THRESH or no_ctx) else "✗")
...
def main():
    ...
    print(f"  C < B : {'✓' if check_cb else '✗ ORDERING BROKEN'}")
    print(f"  B < A : {'✓' if check_ba else '✗ ORDERING BROKEN'}")

    if check_cb and check_ba:
        print("\n  ✓ Tier ordering correct — attention model is working end-to-end")
    else:
        print("\n  ⚠ Tier ordering violated — check Markov params or API context handling")
        all_passed = False
    ...
    if failed == 0 and check_cb and check_ba:
        ...
        print(f"  ⚠ {failed}/{total_checks} checks failed. "
